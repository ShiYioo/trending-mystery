...
// 应用 02 · 档案检索：真实搜索 → 线索卡（翻面/星级/线人低语/归档标记/拖拽）→ 证据投放台 + 档案袋。
// 由房间二移植：接受 seed（案卷检索词预填并自动检索），出口改为 OS 内开窗。
...
export default function SearchApp({ open, seed }: AppProps & { seed?: { keyword: string; nonce: number } }) {
  ...
  useEffect(() => {
    if (seed?.keyword && caseBrief) {
      setKeyword(seed.keyword);
      void run(seed.keyword);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [seed?.nonce, caseBrief?.caseId]);
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1fr_280px]">
      <div className="space-y-5">
        {/* 档案检索 */}
        <section className="paper-card scanline p-5 md:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="eyebrow"><span className="status-dot" />ARCHIVE QUERY / 档案检索</p>
              <p className="mt-2 font-[family-name:var(--font-dossier)] text-[10px] tracking-widest text-paper-600">真实数据流 · ONLINE</p>
            </div>
          </div>
          ...
          >
            <input
              className="input-detective flex-1 px-4 py-2.5 font-[family-name:var(--font-dossier)] text-sm"
              placeholder="输入关键词，调取知乎站内真实回答……"
              value={keyword}
              onChange={(e) => setKeyword(e.target.value)}
            />
            <button type="submit" disabled={loading || !keyword.trim()} className="btn-brass px-5 py-2.5 font-[family-name:var(--font-dossier)] text-sm tracking-widest">
              {loading ? "检索中…" : "检 索"}
            </button>
          </form>
        ...
        {results && results.length === 0 && !loading && (
          <p className="paper-card p-5 text-center text-sm text-paper-600">
        ...
        <div className="space-y-4">
          {results?.map((card, i) => (
            ...
            >
              <TiltCard className={`evidence-card paper-card p-4 ${isCollected(card.contentId) ? "border-brass-400/50" : ""}`}>
                {flipped.includes(card.contentId) ? (
                  <div className="min-h-32 border border-signal-400/20 bg-ink-950/35 p-4">
                    <p className="eyebrow">REVERSE / 证据背面</p>
                    <p className="mt-3 font-[family-name:var(--font-dossier)] text-xs leading-loose text-paper-400">来源：{card.author}<br />互动量：{card.votes.toLocaleString()} 票<br />证据等级：{card.cardId ? "核心证据" : "外围情报"}</p>
      ...
      <aside className="space-y-5">
        <section className="paper-card evidence-drop p-4" onDragOver={(event) => event.preventDefault()} onDrop={(event) => {
          ...
          <div className="mt-3 flex min-h-20 items-center justify-center text-center">
            <p className="max-w-44 font-[family-name:var(--font-dossier)] text-xs leading-relaxed tracking-wider text-paper-600">拖动线索卡至此<br /><span className="text-signal-300">收入实体档案袋</span></p>
          </div>
        </section>
        ...
        <section className="paper-card p-4">
          ...
          <button
            ...
            className="btn-brass mt-4 block w-full px-4 py-2 text-center font-[family-name:var(--font-dossier)] text-xs tracking-widest"
          >
            带证物进审问室 →
          </button>
        </section>
