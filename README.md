...
全部内容接口统一 `{ Code, Message, Data }` 信封与错误码（20001 鉴权失败 / 30001 限频 / 90001 内部错误），client 层统一分类处理，30001 联动配额计数。字段规格以官方 Skill 附带 `http-api.md`（核验 2026-07-16）为准。
...
**搜索配额消化——按关键词计费，不按玩家计费：**

1. 案件生成时预热建议检索词（约 10 次/案）；
2. 缓存键 `(caseId + 归一化关键词)` 全服共享，玩家搜词高度趋同，命中率天然走高；
3. 关键词归一化（去空格 / 井号 / 统一大小写），避免变体重复消耗；
4. 配额耗尽只出缓存内结果，未命中给"档案库检索受限"的剧情化降级，游戏不中断。
