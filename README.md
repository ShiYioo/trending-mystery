...
- [ ] **M0 · 技术验证**：跑通 热榜 → 案件 → 搜索 → 线索卡 最小链路；结案管线数据结构定稿；首日验证四件事——直答 Agent 角色扮演保真度（system prompt / 出示线索卡的响应）、结构化 JSON 输出稳定性（无 JSON mode 则容错解析）、搜索返回实测（ContentType 枚举 / 精选评论条数上限 / AuthorityLevel 取值范围）、OAuth 授权链路（回调域名与测试账号白名单）
