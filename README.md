...
| 身份 | **知乎 OAuth 登录优先，匿名侦探兜底** | 登录人数是人气奖评定指标：进入游戏引导一键授权，解锁真实头像 / 榜单留名 / 海报身份；拒绝或授权失败可匿名继续，评委零障碍；提交材料附测试账号；**未配置真实凭证时自动切换为同协议领域模型的本地模拟授权**（authorize 页 / authorization_code 回调 / 换 token / 用户资料一一对应，前端与回调零改动） |
...
| 模块 | 职责 | 关键设计 |
| --- | --- | --- |
| **案件生成** | 拉取当日热榜，筛选适合做案的问题，生成案情简报、争议点与立场牌底表 | 热点类型过滤器（排除敏感/纯时效话题）；素材丰富度校验（预热搜索返回过薄自动顺延榜上下一题）；同趟 LLM 调用一并完成共识聚类（见下） |
| **取证 / 线索卡引擎** | 代理玩家搜索，把真实回答封装为线索卡 | 可信度星级 = f(RankingScore, AuthorityLevel, VoteUpCount) 加权换算，映射 1–5 星；字段映射见 4.6 |
...
| 审问对峙 | 直答 `POST /v1/chat/completions` | 未标注 | 人设 + 已出示线索卡随请求注入；`zhida-fast-1p5` 档 SSE 流式输出（打字机）；控制历史长度 |
| 结案陈词 | 搜索缓存复用 + 直答 | — | 聚类素材来自缓存；结案叙事用 `zhida-thinking-1p5` 档保质量 |

全部内容接口统一 `{ Code, Message, Data }` 信封与错误码（20001 鉴权失败 / 30001 限频 / 90001 内部错误），client 层统一分类处理；**30001 不做本地配额账本**——实测（2026-09-13）直答端点不消费 `system` 角色消息、内容接口的限频以真实报错为准，自管计数只会把真错误吞成空结果，因此 30001 一律透传为 `zhihu_rate_limited`（429 + 提示更换密钥）。字段规格以官方 Skill 附带 `http-api.md`（核验 2026-07-16）为准。
...
| 原文链接 | `Url` | 自带 utm 溯源参数，用于卡片"查看原文"与海报回流 |
| 线人爆料（M2） | `CommentInfoList` 精选评论 | 仅含 `Content` 文本，无作者 / 赞数 / 权威等级——匿名线人卡天然不可评星，叙事与数据结构自洽 |

单次搜索上限 10 条（`Count` 最大 10，无翻页）——线索墙单次铺 10 张候选卡；案件预热 8~10 个检索词即得约 80~100 张候选卡池，密度和余量都合适。
...
4. 知乎侧真实限频（30001）透传为 `zhihu_rate_limited` 明确报错——运营即时更换密钥，而不是把真错误吞成空结果。
...
| `case:{caseId}` | 案件底表（聚类权重等结案翻牌数据，24h） |
...
- [ ] **M0 · 技术验证**：跑通 热榜 → 案件 → 搜索 → 线索卡 最小链路；结案管线数据结构定稿；首日验证四件事——直答 Agent 角色扮演保真度（system prompt / 出示线索卡的响应）、结构化 JSON 输出稳定性（直答仅保证三个请求字段，提示词约束 + 容错解析为必做）、搜索返回实测（ContentType 枚举 / 精选评论条数上限 / AuthorityLevel 取值范围）、OAuth 授权链路（回调域名与测试账号白名单）
- [ ] **M1 · MVP**：四步玩法循环完整可玩（开局聚类 + 查表算分）+ 知乎 OAuth 登录（登录人数计入人气奖）
- [ ] **M2 · 体验打磨**：审问手感与矛盾检测、陈词一致分、结案报告 + 分享海报、线人爆料（精选评论）、新手教学关（知乎故事改编，避开敏感话题）
- [ ] **M3 · 社区化**：关注流好友动态、侦探档案与等级成长、外线情报（知乎共识 vs 全网风向）
