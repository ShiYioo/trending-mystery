...
**简的（内功，够用即停）**：一个进程、一个 Redis、接口薄封装带缓存、配额一个 INCR、结案查表算分、OAuth 登录 + 匿名兜底、一台服务器一个 pm2。
...
- **微服务、PostgreSQL、第二个模型平台**：全部出局。

### 提交要求对照（网页类项目）

| 官方提交要求 | 我们的方案 |
| --- | --- |
| 公网可访问的线上 Demo 链接 | 单服务器部署（见 4.2），提交前跑通全流程自测 |
| 推荐接入知乎 OAuth——**登录人数计入人气奖** | 登录优先策略（见 4.2）：进入游戏引导一键授权，匿名可兜底继续 |
| 设有登录功能需提供测试账号密码 | 提交材料随附测试账号，demo 日备用 |
...
- [ ] **M0 · 技术验证**：跑通 热榜 → 案件 → 搜索 → 线索卡 最小链路；结案管线数据结构定稿；首日验证四件事——直答 Agent 角色扮演保真度（system prompt / 出示线索卡的响应）、结构化 JSON 输出稳定性（无 JSON mode 则容错解析）、搜索单次返回量实测、OAuth 授权链路（回调域名与测试账号白名单）
